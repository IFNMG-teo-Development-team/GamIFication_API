...
from sql_app.crud import create_user, get_user_by_id_social, user_data
...
@router.post("/", status_code=status.HTTP_200_OK, response_model=UserToken,
             summary="Realiza o login do usuário na API")
async def auth_user(db: Session = Depends(get_db), user: schemas.UserCreate = None):
    ...
    a = user_data(db, auth.idUser)
    return UserToken(
        idUser=auth.idUser, XP=a.XP, Nivel=a.Nivel, type=auth.type, given_name=auth.given_name,
        family_name=auth.family_name, email=auth.email, id_social=auth.id_social,
